...
def database_test(data):
    ...
    cursor.execute("SELECT * FROM LesSportifs WHERE pays = 'Japon' AND numSp = 1192")
    rows = cursor.fetchall()
    ...
    print("\nteste delete on cascade - Discipline\n")
    cursor.execute("SELECT * FROM Discipline WHERE nomDi = 'Patinage artistique'")
    rows = cursor.fetchall()
    print(" AVANT DELETE Patinage DISCIPLINE ")
    for i in rows:
        print(i)
    print("\n")
    cursor.execute("SELECT * FROM LesEpreuves WHERE nomDi = 'Patinage artistique'")
    rows = cursor.fetchall()
    print(" AVANT DELETE Patinage Epreuves ")
    for i in rows:
        print(i)
    print("\n")
    cursor.execute("SELECT * FROM Resultat WHERE numEp IN (SELECT numEp FROM LesEpreuves WHERE nomDi = 'Patinage artistique')")
    rows = cursor.fetchall()
    print(" AVANT DELETE Patinage RESULTATE ")
    for i in rows:
        print(i)
    print("\n")
    cursor.execute("""DELETE FROM Discipline WHERE nomDi = 'Patinage artistique'""")
    cursor.execute("SELECT * FROM Discipline WHERE nomDi = 'Patinage artistique'")
    rows = cursor.fetchall()
